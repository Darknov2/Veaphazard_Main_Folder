...
public class SceneLoadManager : MonoBehaviour
{
    ...
    public static SceneLoadManager Instance
    {
        get
        {
            if (_instance == null)
            {
                // Use compatibility wrapper to avoid deprecated API warnings
    ...
    private IEnumerator UnloadPreviousThenLoad(string sceneAssetName, string menuSceneNameToUnload, int indexHint = -1)
    {
        ...
        if (requireSingleReplace)
        {
            ...
            try
            {
                // replaced deprecated call with SceneFind
                var selector = SceneFind.First<WorldSelector_PersistScenes>();
                selector?.PersistMappingToGameStatePublic();
            }
            catch { }
