...
public class TerrainChunk : MonoBehaviour
{
    ...
    /// <summary>
    /// Apply construction carving to this chunk by re-sampling densities with nearby construction colliders.
    /// This method is called via reflection from ProceduralTerrainGenerator.
    /// </summary>
    public void ApplyConstructionCarving(Collider[] nearbyColliders)
    {
        if (cfg == null || sampler == null)
        {
            Debug.LogWarning("[TerrainChunk] ApplyConstructionCarving called before Initialize.");
            return;
        }
        ...
        // Re-allocate density array if needed
        nx = sizeXZ + 1;
        ny = sizeY + 1;
        nz = sizeXZ + 1;
        int needed = nx * ny * nz;
        ...
        // Marching cubes (same as Generate)
        for (int y = 0; y < sizeY; y++)
        {
            int y0 = y, y1 = y + 1;
            for (int z = 0; z < sizeXZ; z++)
