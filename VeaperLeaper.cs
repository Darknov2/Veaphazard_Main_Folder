...
/// 
/// Changes included:
/// - Upward jump force scales with player's Y relative to the leaper (clamped between min/max inspector values).
/// - Improved agent handling on jump/landing to avoid agent position desync that could stop movement after landing.
/// </summary>
...
public class VeaperLeaper : MonoBehaviour
{
    ...
    [Tooltip("Baseline/upward force used if no player is available.")]
    ...
    [Header("Upward Force Scaling by Player Height")]
    [Tooltip("Minimum upward force applied when player is at or below minHeightDelta relative to this enemy.")]
    public float minUpwardForce = 4f;
    [Tooltip("Maximum upward force applied when player is at or above maxHeightDelta relative to this enemy.")]
    public float maxUpwardForce = 10f;
    [Tooltip("If playerY - enemyY <= minHeightDelta => use minUpwardForce.")]
    public float minHeightDelta = 0f;
    [Tooltip("If playerY - enemyY >= maxHeightDelta => use maxUpwardForce.")]
    public float maxHeightDelta = 6f;
    ...
    // remember agent updatePosition state while jumping
    private bool _agentUpdatePositionWasEnabled = true;
    ...
    void Awake()
    {
        ...
        if (_agent != null)
        {
            ...
            _agentUpdatePositionWasEnabled = _agent.updatePosition;
        }
    ...
    private void TryJumpTowards(Vector3 targetPos)
    {
        ...
        // Disable/bypass NavMeshAgent while airborne and remember updatePosition state
        ...
        // Use computed upward force that scales with player's Y position relative to this enemy.
        float upwardForce = ComputeUpwardForceForPlayer();

        Vector3 impulse = Vector3.up * Mathf.Max(0f, upwardForce)
    ...
    // Compute upward force: maps player's Y delta (playerY - enemyY) into [minUpwardForce, maxUpwardForce]
    // If no player assigned, fallback to jumpUpwardForce for compatibility.
    private float ComputeUpwardForceForPlayer()
    {
        if (_player == null) return jumpUpwardForce;

        float heightDelta = _player.position.y - transform.position.y;

        // Normalize heightDelta into [0,1] across [minHeightDelta, maxHeightDelta]
        float t = Mathf.InverseLerp(minHeightDelta, maxHeightDelta, heightDelta);

        float force = Mathf.Lerp(minUpwardForce, maxUpwardForce, t);

        // Safety fallback
        if (force <= 0f)
            return Mathf.Max(0f, jumpUpwardForce);
    ...
    private void EndJump()
    {
        ...
        if (_agent != null)
        {
            // synchronize agent position with current transform
            ...
            // restore updatePosition and re-enable agent
            _agent.updatePosition = _agentUpdatePositionWasEnabled;
            _agent.enabled = true;
            ...
            // If the player is visible, ensure the agent actively moves toward them
